...
export async function generateMarketingScript(
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a professional marketing copywriter specializing in short, impactful product video scripts.",
        },
        {
          role: "user",
          content: prompt,
    ...
    // Ensure there's content in the response
...
function createScriptPrompt(params: ScriptParams): string {
  return `Write a video script for a ${params.productType} product named "${
    params.productName
  }". 
  ${
...
function validateScript(script: any): asserts script is Script {
  if (!script.headline || typeof script.headline !== "string") {
    throw new Error(
      "Invalid script format: headline is missing or not a string"
    );
  }

  if (!Array.isArray(script.bullets) || script.bullets.length !== 3) {
    throw new Error(
      "Invalid script format: bullets must be an array of 3 items"
    );
  }
  ...
  if (!Array.isArray(script.colorPalette) || script.colorPalette.length !== 3) {
    throw new Error(
      "Invalid script format: colorPalette must be an array of 3 hex colors"
    );
  }
}
